...
- outputView class
  ...
  + [0] 당첨 통계, 수익률 출력하는 기능
- inputView class
  + [0] 로또 구입 금액을 입력 받는 기능 + 로또 개수 출력
  + [0] 로또 당첨 번호를 입력 받는 기능
  + [0] 보너스 번호를 입력 받는 기능

### model package
- validation class
  + [] 지불 금액에 따라 받을 로또 갯수를 구하는 기능
  + [] 당첨 통계 값을 구하는 기능
  + [] 총 수익률을 구하는 기능
- transform class
  + [] 랜덤한 6자리 숫자를 생성하는 기능 (중복 제어)
  + [] 랜덤 6자리 숫자를 인수로 하는 list 생성 및 값 저장 기능
  + [] 입력된 string 형식의 값을 list로 반환하는 기능 // vector 활용할 것

### controller package
  + [] model, view를 통해 로또 진행

### utility package - 구현 완료 후 제작 할 것
...
- userInputException class
  ...
  + [] 로또 당첨 번호 입력에 중복이 있을 경우 - 재입력
  + [] 로또 당첨 번호에 숫자외 값이 올경우 -재입력
  + [] 보너스 번호에 숫자 외의 값이 올 경우 - 재입력
  + [] 보너스 번호 입력에 1~45외의 값이 올 경우 - 재입력
